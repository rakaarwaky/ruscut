    ...
    fn get_video_fps(&self, input_path: &Path) -> f32 {
        let output = Command::new("ffprobe")
            .args([
                ...
                "-of", "default=noprint_wrappers=1:noclose=1=1",
...
impl VideoProcessorPort for FfmpegVideoAdapter {
    fn video_process_video(
        ...
        println!("{} Detected framerate: {} FPS", "INFO:".blue().bold(), format!("{:.2}", fps).yellow());
        ...
        std::fs::create_dir_all(&temp_dir).context("Failed to create temporary directory for video frames")?;
        ...
        println!("{} Processing {} frames using BRIA RMBG-2.0...", "INFO:".blue().bold(), total_frames.to_string().yellow());
        ...
        if is_sequence {
            ...
            println!("{} Encoding transparent video to WebM...", "INFO:".blue().bold());
            ...
            let encode_status = Command::new("ffmpeg")
                .args([
                    ...
                    "-i", &processed_dir.join("frame_%04d.png").to_string_lossy(),
                    "-i", &input_path.to_string_lossy(),
                    "-map", "0:v",
                    "-map", "1:a?",
                    "-c:v", "libvpx-vp9",
            ...
            if !encode_status.success() {
                anyhow::bail!("FFmpeg encoding failed. WebM VP9 might not be fully supported by this installation.");
            }
