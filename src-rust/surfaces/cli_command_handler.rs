...
use crate::taxonomy::removal_types_vo::{
    ModelType, RemovalOptions, get_default_output_path, is_video_path,
};
use clap::Parser;
...
impl CliCommandHandler {
    ...
    fn run_doctor_diagnostics() -> anyhow::Result<()> {
        ...
        let cache_status = if cache_dir.exists() || std::fs::create_dir_all(&cache_dir).is_ok() {
            ...
            format!(
                "{} (Path: {:?})",
                "Failed to write/create".red().bold(),
                cache_dir
            )
        };
        ...
        let vulkan_available =
            crate::infrastructure::vulkan_compute_provider::VulkanComputeEngine::new().is_ok();
        println!(
        ...
        println!(
            "    - Status: {}",
            if vulkan_available {
                "AVAILABLE (Direct RDNA2 hardware compute ready)"
                    .green()
                    .bold()
            } else {
                "UNAVAILABLE (Falling back to simulated CPU)"
                    .yellow()
                    .bold()
            }
        );
    ...
    pub fn run(&self, orchestrator: &BgRemoverOrchestrator) -> anyhow::Result<()> {
        ...
        if args.input.to_string_lossy() == "benchmark" {
            ...
            println!(
                "                 🏁 {}",
                "BENCHMARK RESULTS (AVERAGE)".bold()
            );
            println!(
                "{}",
                "----------------------------------------------------------".blue()
            ...
            println!(
                "  - Engine Selected     : {}",
                report.engine_name.as_str().cyan().bold()
            );
            println!(
                "  - Preprocessing (CPU) : {:.2?}  <-- Resizing + normalization",
                report.preprocess_duration
            );
            println!(
                "  - AI Inference        : {:.2?}  <-- Model execution pass",
                report.inference_duration
            );
            println!(
                "  - Postprocessing      : {:.2?}  <-- Sigmoid + mask scaling",
                report.postprocess_duration
            );
            println!(
                "  - Mask Application    : {:.2?}  <-- Sharpening + alpha blend",
                report.mask_duration
            );
            println!(
                "{}",
                "----------------------------------------------------------".blue()
            ...
            println!(
                "  - {} per frame : {}",
                "TOTAL TIME".bold(),
                format!("{:.2?}", report.total_duration).green().bold()
            );
            println!(
                "  - FPS (Frames Per Sec): {}",
                format!("{:.2}", report.fps).cyan().bold()
            );
            println!(
                "{}",
                "==========================================================".blue()
        ...
        let output_path = args
            .output
            .clone()
            .unwrap_or_else(|| get_default_output_path(&args.input));
        ...
        if is_video_path(&io.options.input_path) {
            println!(
                "{} Starting video background removal...",
                "INFO:".cyan().bold()
            );
